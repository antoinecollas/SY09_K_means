  ...
  for (i in 1:n_ess) {
    ...
    while((delta>epsilon) && (i<n_iter_max)){
      ...
      for (k in 1:K) {
        ...
        #print("det(V_tilde)")
        #print(det(V_tilde[,,k]))
        #print(eigen(V_tilde[,,k])$values)
        #print("Y")
        #print(mu[k,])
        #print("M")
        #print(solve(V_tilde[,,k], tol=1e-20))
        #print("V_tilde[,,k]")
        #print(V_tilde[,,k])
      ...
      for(k in 1:K){
        ...
        temp2 <- t(temp1)
        #print(temp2[,1] %*% t(temp2[,1])) #vérif
        temp3 <- apply(temp2,2,function(x) x%*%t(x))
        #print(matrix(temp3[,1], nrow = p, ncol = p)) #vérif
...
K=2
clust_euclidian <- kmeans(X, K, nstart = 10)
...
clust_adaptative <- K_means_distance_adaptative(X, K=K, n_iter_max = 100, n_ess = 10)
...
#png(filename="imgs/K_means_choix_K.png")
plot(clust_euclidian_inertia, ylab="Inertie intraclasse", xlab = "K", main = "Inertie intraclasse en fonction du nombre de classes choisi.")
...
#png(filename="imgs/K_means_choix_K.png")
plot(clust_adaptative_dist, ylab="Distance", xlab = "K", main = "Distance en fonction du nombre de classes choisi.")
```
...
K=3
clust = kmeans(X, K)
clust_adapt <- K_means_distance_adaptative(X, K=K, rho = rep(1,K), n_iter_max = 100, n_ess = 10)
plot(X, col=z, pch=clust$cluster)
plot(X, col=z, pch=clust_adapt$partition)
print(adjustedRandIndex(z, clust$cluster))
print(adjustedRandIndex(z, clust_adapt$partition))
```
