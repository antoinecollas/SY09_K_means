...
K_means_distance_adaptative <- function(X, K, rho=rep(1, K), n_iter_max=100, n_ess=1, epsilon=10**(-5)) {
  X = as.matrix(X)
  if (K > nrow(X)){
  ...
  for (i in 1:n_ess) {
    ...
    while((delta>epsilon) && (i<n_iter_max)){
      ...
      for (k in 1:K) {
        distances_au_centre_mu_k = distXY(X, mu[k,], solve(V_tilde[,,k]))
        if (k>1){
    ...
    for (k in 1:K){
      mask <- partition==k
      temp <- apply(X[mask,], 1, function(x) distXY(X = t(x), Y = mu[k,], M = solve(V_tilde[,,k])))
  ...
  print("res_n_essai=")
  print(i)
  return(c(min_J_star, res_n_iter, res_partition, res_mu, res_V_tilde))
}
```

```{r}
res <- K_means_distance_adaptative(X, K=3, n_iter_max = 100, n_ess = 10)
# print("min_J_star=")
# print(res[1])
# print("res_n_iter=")
# print(res[2])
